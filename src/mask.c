    ...
    else if(mode != ONLY_ASTIRICS)
    {
            ...
            if(is_dot_exist) {
                ...
                while (pos_last_dot != 1) {
                    if (raw_data[pos_last_dot] == '.') {
                        pos_else_dot[count_subs] = raw_data[pos_last_dot];
                        raw_data[pos_last_dot] = sub;
                        associate[count_subs] = pos_last_dot;
                        count_subs++;
                        some_dots = true;
                    }
                    pos_last_dot--;
                ...
                while (parts_of_mask && parts_of_raw) {
                    if (!mask_func(parts_of_raw, parts_of_mask)) {
                        for (int i = 0; i < count_subs && some_dots; i++) {
                            raw_data[associate[i]] = pos_else_dot[i];
                        }
                        return false;
                    }
        ...
        if(mode == MERGE_Q_AND_A){
            ...
            if(is_dot_exist){
                ...
                for(int i = 0; i < raw_size && i < mask_size; i++){
                    if(mask[i] == '?' || mask[i] == '*'){
                        break;
                    }
                ...
                if(first_part){
                    ...
                    int count_q = 0;
                    for(int i = 0; i < pos_last_dot_mask+1; i++){
                        if(mask[i] != '*'){
                            count_q++;
                        }
                    }
